...
export default function CinematicHero() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // Explicit fromTo on every tween so FROM state is always correct
      // regardless of where the load animation is when this runs
      pinTL
        .fromTo(buildingRef.current, { yPercent: 50 }, { yPercent: 10,    ease: "none" }, 0)
        .fromTo(cloudLRef.current,   { x: 0 },         { x: -250, y: -60, ease: "none" }, 0)
        .fromTo(cloudRRef.current,   { x: 0 },         { x:  250, y: -60, ease: "none" }, 0)
        // Sky travels 100 units (60→-40) so it rises at same visual pace as building
        .fromTo(skyRef.current,      { yPercent: 60 },  { yPercent: -40,   ease: "none" }, 0);

      // Text: fromTo with explicit FROM so it starts visible regardless of load timing
  ...
  return (
    ...
    >
      ...
      <div style={{ position: "absolute", inset: 0, overflow: "hidden", background: "#B8D4E8", isolation: "isolate" }}>

        {/* z=0 — Background */}
        <img src={bgSrc} alt="" style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover", zIndex: 0 }} />

        {/* z=1 — Text: sits behind building so building rises in front */}
        <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "flex-start", justifyContent: "center", paddingTop: "clamp(7rem, 13vh, 14vh)", zIndex: 1, pointerEvents: "none" }}>
          <div ref={textRef} style={{ textAlign: "center", width: "90%", maxWidth: "960px", pointerEvents: "auto", willChange: "transform, opacity" }}>
        ...
        <div style={{ position: "absolute", bottom: 0, left: "50%", transform: "translateX(-50%)", zIndex: 2, width: "115vw", pointerEvents: "none" }}>
          <img ref={buildingRef} src={buildingSrc} alt="" style={{ display: "block", width: "100%", height: "auto", willChange: "transform" }} />
        </div>

        {/* z=10 — Clouds: in front of building */}
        <img ref={cloudLRef} src={cloud1Src} alt="" style={{ position: "absolute", left: "-8%", bottom: "22%", width: "55%", zIndex: 10, filter: "blur(0.6px)", willChange: "transform, opacity" }} />
        <img ref={cloudRRef} src={cloud2Src} alt="" style={{ position: "absolute", right: "-8%", bottom: "22%", width: "55%", zIndex: 10, filter: "blur(0.6px)", willChange: "transform, opacity" }} />
        ...
        <img
          ...
          src={skySrc}
          alt=""
          style={{ position: "absolute", bottom: 0, left: 0, width: "100%", height: "55vh", objectFit: "cover", objectPosition: "bottom center", zIndex: 20, willChange: "transform" }}
        />
